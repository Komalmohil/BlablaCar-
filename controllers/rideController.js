...
exports.renderRideForm=(req,res)=>{ res.render('ride', { isLoggedIn:req.isLoggedIn,username:req.username,userId:req.userId });};
...
exports.showBookingForm=async(req,res)=>{
  try {
    const {rideId}=req.params;
    const ride=await Ride.findById(rideId).populate("publisher","username");

    if (!ride) return res.status(404).send("Ride not found");
    res.render("bookRide",{ride,isLoggedIn:req.isLoggedIn,userId:req.userId,username:req.username});
  }catch(err){
